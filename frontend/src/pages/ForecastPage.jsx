...
export default function ForecastPage({ forecastData, branchId }) {
  ...
  const categories = ['ALL', 'Phở & Bún', 'Cơm & Bánh Mì', 'Ăn Vặt', 'Cà Phê', 'Trà & Trái Cây'];
  ...
  const chartData = currentDish?.daily_forecasts?.map((day) => ({
    ...
    XGBoost: day.xgb_forecast,
    Baseline: day.baseline_forecast,
    Preorders: day.confirmed_preorders,
    Expected: day.expected_demand,
  })) || [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-800 border border-purple-200 flex items-center gap-1">
              <Sparkles className="w-3 h-3 text-purple-600" /> XGBoost Time-series ML
            </span>
          </div>
          <p className="text-sm text-slate-500 mt-1">
            Mô hình Machine Learning phân tích chu kỳ thứ, ngày lễ và xu hướng để dự báo chính xác số lượng từng món.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-xl text-xs text-emerald-800">
          ...
          <span>Độ chính xác mô hình: <strong>WAPE 5.13%</strong> (Đã test trên 110 ngày thực tế)</span>
        </div>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-2.5 px-2 text-center bg-emerald-50/50 align-middle">
                      <span className="text-xs font-bold text-emerald-700">{day1.expected_demand}</span>
                      {day1.confirmed_preorders > 0 && (
                        <span className="block text-[9px] text-blue-600 font-semibold">
                          (Đặt: +{day1.confirmed_preorders})
                        </span>
                      )}
                    </td>
